...
farbeEinkommen <- tapply(X = einkommen, INDEX = augenfarbe, FUN = function(x) mean(x))
...
levels(mehraugen) <- c(levels(mehraugen), "nussbraun")
mehraugen[10] <- "nussbraun"
...
personenMitFarbe <- function(namen, farben) {id <- which.max(table(farben))
...
mmmmEinkommen <- tapply(einkommen, augenfarbe, FUN = function(x) {c(min(x), median(x), mean(x), max(x))})
mmmmEinkommen
# $blau
#   1500   1750   1750   2000 
# $braun
